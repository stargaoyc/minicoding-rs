...
//! 断言其 `[dependencies]` 与 `[dev-dependencies]` 均不含白名单外的 `minicoding-*`
//! crate——把"领域互不依赖"从文档约束升级为 CI 强制。dev-deps 与 deps 同权重：
//! 测试通道的依赖边同样是真实编译期耦合，同样会阻碍未来拆分。只检查 workspace
//! 内部边；平台/外部重依赖由各 crate 的 feature gate 治理（见 modules.md §0.4），
//! 不在本守卫范围。

/// 断言当前 crate（以测试 cwd = crate 根为准）的 `[dependencies]` 与
/// `[dev-dependencies]` 中，所有 `minicoding-*` 依赖都在 `allowed` 白名单内。
///
/// # Panics
...
pub fn assert_workspace_deps(allowed: &[&str]) {
    ...
    for section in ["dependencies", "dev-dependencies"] {
        // [dependencies] 必须存在（crate 必然依赖 core）；[dev-dependencies] 可缺省。
