...
- **工具系统**：基于 `Tool` trait 的注册式工具，内置文件操作、Bash、Grep/Glob、Web 抓取、TodoWrite、Plan、MCP 远程工具等。
...
- **OS 级沙箱（一等公民）**：macOS Seatbelt、Linux Landlock+seccomp、Windows 受限令牌，作为应用层权限之外的内核级第二道防线；支持 `external-sandbox`（CI/容器）与 `danger-full-access`（显式确认）。
- **Hooks 系统（参考 Claude Code）**：8 类生命周期事件（PreToolUse/PostToolUse/PermissionRequest/...），外部脚本 + JSON over stdio 协议，可拦截/改写/注入；L0 硬约束不可被 Hook 覆盖。
...
│   └── rules.md                # 设计时大模型约束
├── crates/
│   ├── minicoding-core/        # 核心运行时 + trait 定义（library）
│   ├── minicoding-sandbox/     # OS 级沙箱驱动（seatbelt/landlock/seccomp/windows）
│   ├── minicoding-providers/   # LLM Provider 实现
│   ├── minicoding-tools/       # 内置工具集（含 todo/plan/file-undo/mcp 包装）
│   ├── minicoding-mcp/         # MCP client/server 实现
│   ├── minicoding-cli/         # CLI frontend
│   ├── minicoding-tui/         # TUI frontend（后续）
...
| [详细设计](docs/design.md) | Agent 循环、上下文管理、工具调度、Plan/Undo/Todo/MCP/Hooks 等核心机制 |
| [模块设计](docs/modules.md) | 每个 crate / 模块的职责与内部结构 |
...
| [Hooks 设计](docs/hooks.md) | 8 类生命周期 Hook、协议、安全约束 |
| [技术选型](docs/tech-stack.md) | 依赖库选择与理由（含沙箱/MCP/Hooks） |
| [开发路线图](docs/roadmap.md) | 里程碑与交付计划 |
| [功能清单](docs/features.md) | 全功能总账（按领域分组，133 项） |
| [大模型约束](docs/rules.md) | 设计时对 LLM 施加的 L0/L1/L2 约束（C-01..C-25） |
