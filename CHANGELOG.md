...
- **思考过程按消息位置内联显示**：reasoning 不再平铺堆到对话末尾——改为
  按消息 id 关联（`reasoningByMessageId`），渲染在对应 assistant 消息的
  实际位置上方
- **运行流程面板思考合并为一段**：`reasoning_delta` 逐 token 到达时合并累积
  为完整段落显示（此前一个字母一条日志，不可读）
- **权限模式切换器显示同步**：新建会话选定"编辑自动/规划/全自动"后，左下角
  切换器仍显示"默认"——创建成功时把所选模式同步到 UI store（此前仅运行时
  切换才更新）
